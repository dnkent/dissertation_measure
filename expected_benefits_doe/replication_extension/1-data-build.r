...
### NOTE: Original Carroll and Kenkel (2019) script, but I drop stalemates
...
data_nmc %>%
    summarise_at(
        vars(one_of(nmc_components, "cinc")),
        ~ mean(is.na(.))
        ) %>%
    print()
...
data_nmc <- data_nmc %>%
    ...
    mutate(
        yearly_total = sum(raw, na.rm = TRUE),
...
data_nmc %>%
    ...
    mutate(
        nmc_sum = irst_prop + milex_prop + milper_prop + pec_prop +
        tpop_prop + upop_prop
    ) %>%
...
data_dir_dyad <- data_nmc %>%
    ...
    mutate(
        data = map(
            data, ~ crossing(ccode_a = .$ccode, ccode_b = .$ccode)
            )
        ) %>%
    unnest() %>%
...
data_mida <- data_mida %>%
    mutate(
        outcome = case_when(
            ...
            #outcome == 5 ~ "Stalemate" # This is the big change
        )) %>%
...
print(with(data_mida, table(outcome, styear)))
...
data_non_dir_dyad <- data_dir_dyad %>% 
    filter(ccode_b > ccode_a)

# Save datasets -- training is small and can be csv
# Save the full dataset for predictions as an rds file because large
write_csv(data_train, path = "results/data-train-small-no-stale.csv")
saveRDS(data_non_dir_dyad, "results/data-non-dir-dyad.Rds")